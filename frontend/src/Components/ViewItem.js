...
const Item = ({item, handleItemDelete, locations, handleItemEdit}) => {
    ...
    const handleSubmit = (e) => {
        ...
        if(location === undefined || location.name === null){
            if(options.length === 0){
                locationName = null
            }else {
            ...
            updateItem.location = location.id === undefined ? location : location.id
            locationName = options.filter(o => (o.value === location || o.value === location.id))[0].label
        }
